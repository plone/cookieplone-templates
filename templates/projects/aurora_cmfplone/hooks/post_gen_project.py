...
def generate_addons_aurora(context: OrderedDict, output_dir: Path) -> Path:
    ...
    npm_package_name = frontend_context.get("npm_package_name", frontend_addon_name)
...
def generate_sub_project_settings(context: OrderedDict, output_dir: Path) -> Path:
    """Configure backend language and project settings without Volto files."""
    settings_context = _fix_frontend_addon_name(context.copy())
    # The shared settings schema still requires this legacy internal key even
    # when its frontend payload is excluded.
    settings_context["volto_version"] = settings_context["frontend_base_version"]
    # Generate into staging first: removing frontend directly from a merge
    # targeting output_dir would also remove the existing Aurora workspace.
    with tempfile.TemporaryDirectory() as temp_dir:
        staged_path = generator.generate_subtemplate(
        ...
        shutil.copytree(staged_path, output_dir, dirs_exist_ok=True)
    return output_dir


def generate_ci_gh_aurora_cmfplone(
    ...
    return generator.generate_subtemplate(
        f"{TEMPLATES_FOLDER}/ci/gh_aurora_cmfplone",
        output_dir,
        ".github",
        ci_context,
        global_versions=versions,
...
SUBTEMPLATE_HANDLERS = {
    ...
    "ci/gh_aurora_cmfplone": generate_ci_gh_aurora_cmfplone,
