...
REPOSITORY_URL = "https://github.com/plonegovbr/plone.org.br"
FRONTEND_PACKAGE = "frontend/packages/volto-ploneorgbr"

# A monorepo project ships three independent towncrier setups: the project
# itself, the backend package and the frontend add-on.
TOWNCRIER_CONFIGS = [
    "towncrier.toml",
    "backend/pyproject.toml",
    f"{FRONTEND_PACKAGE}/towncrier.toml",
]

NEWS_FOLDERS = [
    "news",
    "backend/news",
    f"{FRONTEND_PACKAGE}/news",
]
...
def test_towncrier_settings(
    ...
    """Test towncrier settings are the same across the three codebases."""
    path = cutter_result.project_path / filename
...
def test_towncrier_types(cutter_result, towncrier_types, filename: str):
    """Test the news fragment types are the same across the three codebases."""
    path = cutter_result.project_path / filename
