...
On each push to such branch, the basic checks (linter) will be executed.

When opening a merge requests from that branch to `main` tests will be executed.


## Automatic deployment 🚀

The deployment is executed automatically when a tag is created in the repository.

The images will be built when creating a tag.

The tag name will be used as the docker image tag and will be used when running the deployment.
